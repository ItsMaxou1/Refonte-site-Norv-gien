...
function afficherImageSuivante() {
  index = (index + 1) % 5; // On change entre les 4 images
  afficherImage(index);
}
...
/* Produit automatique dans le panier avec le prix et tout */
function ajouterAuPanier(nomProduit, prixProduit, imageProduit) {
  // Récupérer le panier du localStorage (ou créer un panier vide s'il n'existe pas)
  ...
  // Rediriger l'utilisateur vers la page de détail du panier
  window.location.href = "Detail-de-commande/detail.html";
}

/* Fin du produit automatique */
